...
DroneS::DroneS(JsonObject& obj) : details(obj) {
...
void DroneS::GetNearestEntity(std::vector<IEntity*> scheduler) {
    ...
    for(auto entity : scheduler){
        ...
        if(droneType.compare("drone_s") == 0 && entity->GetAvailability() && entity->GetCarrier() == NULL && distanceRobotDestination > 25.5){
...
void DroneS::Update(double dt, std::vector<IEntity*> scheduler) {
    if(available) {
...
void DroneS::Rotate(double angle){
